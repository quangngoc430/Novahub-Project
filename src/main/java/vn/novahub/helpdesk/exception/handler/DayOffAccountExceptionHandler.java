...
@ControllerAdvice
public class DayOffAccountExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleDayOffTypeNotFoundException(HttpServletRequest request, Exception ex){
        String message = "Day off account not found";
    ...
    public ResponseEntity<ApiError> handleDayOffAccountIsExistException(HttpServletRequest request, Exception ex){
        String message = "Day off account is already exist";
        HashMap<String, String> errors = new HashMap<>();
        errors.put("message", message);
